...
int isAnagram3(char *w1, char *w2)
{
  ...
  {
    if (isspace(w1[i]))
      continue;  /* >>>>> a statement s1 goes here <<<<< */
    no_chars_w1++;
    sum_w1 += tolower(w1[i]);/* >>>>> an expression e1 goes here <<<<< */ ;
  }
  for (i = 0; w2[i] ; ++i)
  {
    if (isspace(w2[i]))
       continue; /* >>>>> s2 goes here <<<<< */
    no_chars_w2++;
    sum_w2 += tolower(w2[i]);/* >>>>> e2 goes here <<<<< */ ;
  }
  if ( sum_w1==sum_w2/* >>> e3 here <<< */ && no_chars_w1==no_chars_w2/* >>> e4 here <<< */)
    return 1;
  else
    return 0;
}
